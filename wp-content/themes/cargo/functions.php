...
if ( ! class_exists( 'CargoTheme' ) ) {
	...
	class CargoTheme {
		...
		function bt_set_global_uri() {
			...
			echo 'window.bt_text = [];';
			echo 'window.bt_text.previous = \'' . esc_html__( 'previous', 'cargo' ) . '\';';
			echo 'window.bt_text.next = \'' . esc_html__( 'next', 'cargo' ) . '\';';		
			echo '</script>';
		}
		...
		function bt_enqueue_scripts_styles() {
			...
			if ( function_exists( 'boldthemes_csscrush_file' ) ) {
				boldthemes_csscrush_file( get_stylesheet_directory() . '/style.css', array( 'source_map' => true, 'minify' => false, 'output_file' => 'style.cargo', 'formatter' => 'block', 'boilerplate' => false,  'plugins' => array( 'loop', 'ease' ) ) );
				boldthemes_csscrush_file( get_stylesheet_directory() . '/style.css', array( 'source_map' => true, 'minify' => true, 'output_file' => 'style.cargo.min', 'boilerplate' => false,  'plugins' => array( 'loop', 'ease' ) ) );
			}
			
			wp_enqueue_style( 'bt_style_css', get_template_directory_uri() . '/style.cargo.min.css', array(), false );
			
			wp_enqueue_style( 'bt_buggyfill_css', get_template_directory_uri() . '/css/viewport-buggyfill.css', array(), false );
			
			// wp_enqueue_style( 'bt_fancy_css', get_template_directory_uri() . '/css/fancySelect.css', array(), false );
			...
			wp_enqueue_script( 'bt_ie9_js1', esc_url_raw( 'http://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.min.js' ), array(), false );
			wp_enqueue_script( 'bt_ie9_js2', esc_url_raw( 'http://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js' ), array(), false );
		...
		function bt_load_fonts() {
			...
			if ( $body_font != 'no_change' ) {
				...
				$body_font_state = _x( 'on', 'Raleway font: on or off', 'cargo' );
			...
			if ( $menu_font != 'no_change' ) {
				...
				$menu_font_state = _x( 'on', 'Lato font: on or off', 'cargo' );
			...
			if ( $heading_font != 'no_change' ) {
				...
				$heading_font_state = _x( 'on', 'Lato font: on or off', 'cargo' );
			...
			if ( $heading_supertitle_font != 'no_change' ) {
				...
				$heading_supertitle_font_state = _x( 'on', 'Raleway font: on or off', 'cargo' );
			...
			if ( $heading_subtitle_font != 'no_change' ) {
				...
				$heading_subtitle_font_state = _x( 'on', 'Raleway font: on or off', 'cargo' );
			...
			if ( count( $font_families ) > 0  ) {
				...
				wp_enqueue_style( 'bt_fonts', $font_url, array(), '1.0.0' );
			}
		}
 ...
 * @return string
 */
if ( ! function_exists( 'bt_get_body_class' ) ) {
	function bt_get_body_class() {
