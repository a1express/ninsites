...
<?php if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		$share_html = '<div class="btIconRow">' . bt_get_share_html( $permalink ) . '</div>';
		...
		echo '<article class="' . implode( ' ', get_post_class( $class_array ) ) . '">';
			echo '<div class="port">';
				...
				echo '<div class="boldCell"><div class = "boldRow">';
					echo '<div class="rowItem col-sm-12">' . bt_get_heading_html( '', '<a href="' . esc_url_raw( $permalink ) . '">' . get_the_title() . '</a>', $meta_html . $categories_html, 'large', 'bottom', '', '' ) . '</div>';
				echo '</div></div>';
				...
				echo '<div class="boldCell"><div class="boldRow btArticleBody topSmallSpaced">
						<div class="rowItem col-sm-12">' . $content_final_html . '<div class="btClear btSeparator topSmallSpaced bottomSmallSpaced border"><hr></div></div>';
				echo '</div></div>';
				}
			...
			echo '<div class="boldCell"><footer class="boldRow">
					...
					<div class="rowItem col-md-6 col-sm-12 btTextRight btGridShare">' . $share_html . '</div> 
				</footer></div>			
			</div><!-- port -->
		</article>';
